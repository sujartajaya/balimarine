...
<div class="max-w-sm w-full bg-red-600/80 p-8 rounded-3xl text-center text-blue-600 shadow-2xl backdrop-blur-md">
    ...
    <!-- ✅ EMAIL TIDAK PERNAH DIHILANGKAN -->
    <input type="email" 
           class="rounded-xl text-gray-800 mt-6 w-full px-6 py-3 border-none focus:ring-2 focus:ring-teal-300"
           name="email" 
           required 
           autofocus 
           id="login_email" 
    ...
    <input type="hidden" id="mac" value="{{ $data['mac'] ?? '' }}">

    <!-- ✅ SATU BUTTON SAJA -->
    <button id="submitBtn"
        class="bg-teal-400 text-indigo-950 font-black px-6 py-3 rounded-xl mt-4 w-full tracking-wide uppercase">
        Sign In
    </button>

    <div class="mt-4">
        <p id="errorInfo" class="font-bold text-yellow-300 text-xs">
            {{ $data['error'] ?? '' }}
        </p>
    </div>
</div>
...
<script>
    ...
    submitBtn.addEventListener('click', async function(e) {
        ...
        const country_id = 96;
        const post_url = "{{ route('webloginv4') }}/store";
    ...
    function loginMikrotik() {
        <?php if (isset($data['chap-id']) && strlen($data['chap-id']) > 0) { ?>
            document.loginweb.password.value =
                hexMD5(
                    '{{ $data['chap-id'] ?? '' }}' +
                    document.loginweb.password.value +
                    '{{ $data['chap-challenge'] ?? '' }}'
